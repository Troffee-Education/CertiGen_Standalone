...
function isHeaderRow(row: unknown[], minCells = 2): boolean {
  if (!row || row.length === 0) return false;
  const text = row.map((c) => String(c ?? "").trim());
  const nonEmpty = text.filter((c) => c.length > 0);
  if (nonEmpty.length < minCells) return false;

  const lowerJoined = text.map(t => t.toLowerCase()).join(" ");
  const hasCommonHeaderKeyword = [
    "name", "nama", "email", "emel", "school", "sekolah", "timestamp", "phone", "telefon", "state", "negeri", "category", "kategori", "ic", "nokp"
  ].some(k => lowerJoined.includes(k));

  if (hasCommonHeaderKeyword) return true;

  const isMostlyStrings = nonEmpty.filter(c => isNaN(Number(c))).length / nonEmpty.length > 0.6;
  return isMostlyStrings;
}
